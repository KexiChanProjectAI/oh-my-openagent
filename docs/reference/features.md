...
The published package is `@zenkexi/oh-my-openagent` and the binary is `oh-my-openagent`. Inside `opencode.json`, the compatibility layer now prefers the plugin entry `oh-my-openagent`, while legacy `oh-my-opencode` entries still load with a warning. Plugin config files (`oh-my-openagent.json[c]` or legacy `oh-my-opencode.json[c]`) are recognized during the transition. Run `bunx @zenkexi/oh-my-openagent doctor` to check for legacy package name warnings.
...
Run `bunx @zenkexi/oh-my-openagent doctor` to see capability diagnostics including:
